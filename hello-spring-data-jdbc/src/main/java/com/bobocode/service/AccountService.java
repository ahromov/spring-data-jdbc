...
/**
 * {@link AccountService} is a simple service that provides some API for
 * {@link Account} entity. It is using {@link AccountRepository} in order to
...
public class AccountService {
	...
	/**
	 * Loads {@link Account} by a given id. Throws {@link AccountNotFoundException}
	 * if Account with given id is not found.
	 *
	...
	public Account getAccountById(Long id) {
		return accountRepository.findById(id).orElseThrow(
				() -> new AccountNotFoundException(String.format("Account with id %d does not exist", id)));
	}
	...
	/**
	 * Finds all accounts that has the given lastName. It uses a special method
	 * implemented by {@link AccountRepository}. FILTERING BY LASTNAME SHOULD BE
	...
	/**
	 * Updates a corresponding account record in the database using by its primary
	 * key (id).
	 *
	...
	/**
	 * Removes a corresponding account record in the database by its primary key
	 * (id).
	 *
	...
	/**
	 * An atomic operation that gives every account with a given lastName a bonus.
	 * In case or an error, no account balances should be updated.
	 *
