...
/**
 * A util tool that helps to build nice test data. It is based on JFairy
 * library.
 */
public class TestDataGenerator {
	...
	public static List<Account> generateAccountList(int size) {
		return Stream.generate(TestDataGenerator::generateAccount).limit(size).collect(toList());
	}
	...
	private static void fillAccount(Account account, Person person) {
		...
		account.setBirthday(LocalDate.of(person.getDateOfBirth().getYear(), person.getDateOfBirth().getMonthOfYear(),
